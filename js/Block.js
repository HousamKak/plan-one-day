...
export class Block {
    ...
    updateLabel() {
      ...
      // Use the title and time range
      titleElement.textContent = `${startTime} - ${endTime}`;
      
      if (this.labelArrow) {
        this.labelArrow.textContent = `${startTime} - ${endTime}`;
      }
      
      if (this.wrapElement) {
        const wrapTitleElement = this.wrapElement.querySelector('.block-title');
        wrapTitleElement.textContent = `${startTime} - ${endTime}`;
      }
    }
    ...
    updateLabelVisibility() {
      ...
      if (blockWidth < titleWidth + 12) {
        ...
        if (!this.labelArrow) {
          ...
          this.labelArrow.textContent = titleElement.textContent;
